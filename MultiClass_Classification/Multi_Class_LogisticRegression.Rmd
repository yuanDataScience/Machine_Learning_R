...
In the following implementation, a matrix called thetas were created. This matrix contains 10 rows and 401 columns. Each row of the thetas corresponds to the learned logistic regression parameters for one class. 
```{r}
...
Furthermore, Rcgmin function in Rcgmin package was used to find the optimum theta. This function uses the conjugated gradient minimization algorithm for the efficient search of the optimum values. In the following R code, we use an intial theta vector with all its element set as 0 for all the classifiers.

Finally, it is important to note that for the one-vs-all classifier, each classifier acts as a "binary" classifer. For example, classifier i only cares if an observation belongs to class i or not. Therefore, when training classifier i, we only need to tell the classifier whether an observation belongs to class i, as shown in the following code. 
...
```
We can see that the dimension of theta1 is 25 by 401. Theta1 is the $\Theta$ matrix from the input layer nodes to the middle layer nodes. As mentioned previously, the number of rows in this matrix should correspond to the number of output nodes, which is 25. In addition, the dimension of theta2 is 10 by 26, corresponding to the number of middle layer nodes (25 plus the biased node) and the 10 output layer nodes.

For multiple layer neural networks, every node except those in the input layer can be computed using logistic regression. Based on this principle, we can implement the third layer (output layer) nodes. The thus computed values of the 10 nodes in the output layer for each training example were the probabilities of that example belonging to the corresponding classes. class with the highest probability were assigned to that example. The R code for the prediction is shown in the following section:
