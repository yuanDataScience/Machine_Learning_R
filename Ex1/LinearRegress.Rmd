...
This exercise implements the linear regression using R. For the detailed information about this assignment, please refer to the Programing Exercise 1: Linear Regression pdf file you obtained from the Machine Learning Coursera course.
...
The Ex1.Rmd markdown file read the data files, organized all of the functions defined in the above list. The corresponding notbook and the markdown files also explained the implementation of the algorithms according to the best knowledge of the author. The data used in this excercise are exdata1.txt and exdata2.txt. These txt files were included in this folder. In addition, instead of implementing a plotData function specific for displying the data, the data visualization was accomplished using ggplot2 package. 
...
**2. Linear regression with one variable **         
   In this part of the exercise, the linear regression will be used to predict the profits for a food truck using the      populations from different cities as the predictor. The prediction of the linear regression will help you to select     which city to expand next based on the population of that city.

__2.1. Plotting the data __     
Data exploration is always the first step to do any data analysis. For this dataset, scatter plot was used to visualize the data. In the following scatter plot, profit was plotted against the population of the cities.
...
$$\theta = \theta - \frac{\alpha}{m} X^T\left(X\theta-Y\right)~~~~~~~~~~~~~~ Eq\left(4\right)$$ 
  ...
  where X is the matrix consisting of observation $x^{\left(i\right)}$ as its ith row, and Y is the vector          consisting of the target variable, with observation, $y^{\left(i\right)}$ as its ith element. 
...
**2.2.2. Implementation **          
 In the next step, we will establish the model by adding the intercept column, $\theta_0$ to the design matrix, profit_datax, initialize the initial parameters including all the elements in $\theta$ as zeros, and set up the learning rate $\alpha$ to 0.01. In addition, values of target variable for all the observations are stored in the vector profit_targety.
...
**2.2.3. Computing the cost $J\left(\theta\right)$ **       
 In this part, the cost function was implemented based on $Eq\left(5\right)$
...
initialCost
```
The output of the cost function when $\theta$ was initialized to zeros was 32.07. This value is constent to the answer given in the assignment. 

**2.2.4. Gradient descent **         
 Next is to implement the gradient descent function. This function was written by the class. Here, the function was implemented in R using the same algorithm. The gradient descent function will run 1500 iterations, and return the costs and theta values for each iteration.
...
```
This plot showed that the value of cost function decreased dramatically with the increase of the number of the iterations, with the learning rate set at 0.01. 

Next, we will predict the price of two houses using the established univariate model.
```{r}
#predict the values
...
```
This plot showed the regression line obtained by the univariate linear regression model we established.

**2.4. Visualization $J\left(\theta\right)$ **

The cost function $J\left(\theta\right)$ was plotted over a 2-dimensional grid of $\theta_0$ and $\theta_1$ values. In R, the gird is generated by expand.grid function. The ranges of $\theta_0$ and $\theta_1$ are [-10,10] and [-1,4], respectively. 100 points were selected for both $\theta_0$ and $\theta_1$. The cost for each pair of the $\theta_0$ and $\theta_1$ combination was calculated by the computeCost function, and the 3-dimensional plot of the cost versus $\theta_0$ and $\theta_1$ was generated using the plot3d function of the rgl package. The 3D plot is shown in a pop-up window, and you can ajust the size of the plot by adjusting the size of the window. 
...
```
In addition to the 3D plot, the contour plot was generated to show the optimum $\theta$ values found by the regression algorithm corresponding to the minimum cost.
...
This part implemented linear regression with multiple variables to predict the prices of houses. The model contains information on houses sold and make a model of housing prices based on the house information.
...
```
After the feature scaling, the difference in the magnitude of size and bedrooms is much smaller.
...
```
From this figure, we can see that when learning rate was 0.3, the cost decreased and converged very fast. Therefore, we will use the theta values obtained by the learning rate of 0.3 in the follwoing steps. Now, we will use the theta obtained from the optimum learning rate to predict the price of a house with 1650 square feet and 3 bedrooms. Remeber that we need to do the feature scaling before the prediction.
...
Based on Eq(6), the OLS solution was implemented by the function normalEqn(design_matrix, targety):
```{r}
